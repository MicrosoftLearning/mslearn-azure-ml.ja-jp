...
多くのシナリオでは、予測モデルを使用して多数のケースをスコアリングするバッチ プロセスとして推論が実行されます。 Azure Machine Learning で、この種の推論ソリューションを実装するために、バッチ エンドポイントを作成できます。

この演習では、MLflow モデルをバッチ エンドポイントにデプロイし、ジョブを送信してサンプル データでテストします。
...
1. **Labs/11/Deploy to batch endpoint.ipynb** ノートブックを開きます。
