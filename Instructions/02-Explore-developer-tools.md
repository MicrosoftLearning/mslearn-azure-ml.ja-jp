...
## Azure Machine Learning スタジオを使用してワーク ステーションを構成する

Azure CLI は自動化に最適ですが、実行したコマンドの出力を確認することもできます。 Azure Machine Learning スタジオを使用して、リソースとアセットが作成されているかどうかを確認したり、ジョブが正常に実行されたかどうかを確認したり、ジョブが失敗した理由を確認したりできます。 
...
1. ジョブの **[プロパティ]** を確認します ジョブの次の**状態**に注目してください。 
    ...
    - **[失敗]** : ジョブは失敗し、終了しています。
1. ジョブが完了すると、 **[メトリック]** の下にモデルの *[AUC]* と *[精度]* が表示されます。 
1. **[出力とログ]** には、次の情報が表示されます。
    - フォルダー内のモデル pickle ファイルが**出力されます**。 
    - **user_logs/std_log.txt** のスクリプトの出力。 スクリプトの **print** ステートメントからの出力がここに表示されます。 スクリプトの問題によりエラーが発生した場合は、こちらにもエラー メッセージが表示されます。
