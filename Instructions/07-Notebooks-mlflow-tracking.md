...
多くの場合、複数のモデルを実験してトレーニングすることで、新しいデータ サイエンス プロジェクトを開始します。 作業を追跡し、トレーニングするモデルの概要とそのパフォーマンスを把握するには、MLflow 追跡を使用できます。

この演習では、コンピューティング インスタンスで実行されているノートブック内の MLflow を使用し、モデル トレーニングをログに記録します。
...
Azure Machine Learning ワークスペースとコンピューティング インスタンスを作成するには、Azure CLI を使用します。 実行するために必要なすべてのコマンドがシェル スクリプトにグループ化されます。
...
1. ターミナルで次のコマンドを実行して、コンピューティング インスタンスに Python SDK と MLflow ライブラリをインストールします。
...
必要なすべてのリソースが揃ったので、ノートブックを実行して、MLflow を構成してノートブックでモデルをトレーニングするときに使用できます。

1. **Labs/07/Track model training with MLflow.ipynb** ノートブックを開きます。
